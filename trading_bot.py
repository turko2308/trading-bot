...
M4_ENABLED       = False   # 3.12.0: מושבת — הצצה לעתיד (DECISIONS §42)
...
# ── 3.9.7: סדר הסריקה — תיקון תצוגה ──────────────────────────
# tf_scan רץ על הרשימה לפי הסדר. כשה-4H היה ראשון, איתות 6H של
# *אותה סריקה* עדיין לא היה ב-log כשה-4H בדק גיבוי — ולכן נר 4H
# שנסגר ב-00:00 או 12:00 (סימולטני עם נר 6H) יצא עם ⚠️ "אין
# גיבוי", ומיד אחריו הודעה שנייה "גיבוי הגיע אחרי 0.0 שעות".
#
# ⚠️ זה *לא* היה באג סיווג. השדה backup בגיסט יצא True בשני
# המקרים — לולאת הגיבוי-המאוחר (למטה, ~2414) תפסה את הסימולטני
# באותה סריקה. נמדד: שיעור הגיבוי 81% לפני ואחרי, הפרש 0
# איתותים. גם השערת ה-cutoff-משעון-הקיר נבדקה: 0 איתותים.
# ר' tools/lookback.py — הוא מתעד את שתי ההשערות שנדחו.
#
# מה שהתיקון כן נותן: ההודעה הראשונה שאיזק קורא היא הנכונה,
# במקום ⚠️ שגוי שמתוקן שניות אחר כך. 2 הודעות במקום 3.
# TF_BACKUP_LOOKBACK_H נשאר 6 — הוא לא היה הבעיה.
TF_CONFIGS = [
    {"name": "6H", "hours": 6, "worst_dd": 6009, "med_dd": 2172,
     "far": {"trades": 164, "wr": "62%", "pnl": "+11,720", "per": "+71.47", "periods": "5/5"}},
    # 4H הושהה 14/09/2026: 32.1% הצלחה, -703.9$ על כל ההיסטוריה (לא רק לאחרונה),
    # אושש מ-tf_signals האמיתיים ב-Gist. נבדק גם על אוג-ספט 2026 בפועל — נכשל שם
    # יותר מכל תקופה אחרת. 6H נשאר (עדיין חיובי במצטבר, +771.95$, גם ברג'ים הקשה).
    # לא נמחק — רק מושבת. להחזיר רק עם בדיקה חדשה, לא סתם.
